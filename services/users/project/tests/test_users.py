...
class TestUserService(BaseTestCase):
    ...
    def test_add_user_invalid_json_keys(self):
        ...
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 400)
            self.assertIn('No username provided.', data['message'])
            self.assertIn('fail', data['status'])
    ...
    def test_single_user(self):
        ...
        user = add_user('eder', 'eder@eder.org', 'pass')
        with self.client:
    ...
    def test_all_users(self):
        ...
        """
        add_user('eder', 'eder@eder.org', 'pass')
        add_user('nicole', 'nicole@nicole.org', 'pass')
        with self.client:
    ...
    def test_main_with_users(self):
        ...
        """
        add_user('eder', 'eder@eder.org', 'pass')
        add_user('ping', 'ping@ping.org', 'pass')
        with self.client:
            response = self.client.get('/')
            self.assertEqual(response.status_code, 200)
            self.assertIn(b'All Users', response.data)
    ...
    def test_add_user_invalid_json_keys_no_password(self):
        ...
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 400)
            self.assertIn('Password must be non-empty', data['message'])
            self.assertIn('fail', data['status'])
